...
Every release publishes a `SHA256SUMS` file alongside the binaries, and `install.sh` checks the downloaded binary against it before installing — the install is aborted, nothing is written to disk, if it doesn't match or `SHA256SUMS` is missing from the release.

Each binary also carries a [signed build provenance attestation](https://docs.github.com/en/actions/security-guides/using-artifact-attestations-to-establish-provenance-for-builds), generated by the release workflow itself and backed by [Sigstore](https://www.sigstore.dev/). If you want cryptographic proof a given binary was actually built by this repo's CI (not just that it matches a hash sitting next to it in the same release), verify it with the [GitHub CLI](https://cli.github.com/):

```bash
gh attestation verify <path-to-binary> --repo Emperorex/artools
```

This isn't required for `install.sh` to work — it's an extra step for anyone who wants it.
